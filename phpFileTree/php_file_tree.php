		...
		foreach( $file as $this_file ) {
			if( $this_file != "." && $this_file != ".." ) {
				if( is_dir("$directory/$this_file") ) {
					...
					$php_file_tree .= php_file_tree_dir("$directory/$this_file", $return_link."&lesson=".$this_file ,$extensions, false);
					$php_file_tree .= "</li>";
				} else {
					// File
					...
					$OutputStyle = (!isset($_GET["style"])) ? "androidstudio.css" : $_GET["style"];
					$show = (!isset($_GET["show"])) ? "output" : $_GET["show"];
					$link = $return_link."&exercise=".urlencode($this_file)."&show=".$show."&style=".$OutputStyle;
